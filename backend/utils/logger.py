...
_LOG_FILE_DIR = Config.get_instance().get("server.log.save_path", "logs/")
...
def get_logger(logger_name: str = "paraformer-web", **kwargs) -> logging.Logger:
    ...
    file_name = kwargs.get("file_name", ("access.log", "error.log"))
...
stdout_format = Config.get_instance().get(
    "server.log.stdout_format", DEFAULT_STDOUT_FORMAT
)
filehandle_format = Config.get_instance().get(
    "server.log.filehandle_format", DEFAULT_FILEHANDLER_FORMAT
)
save_path = Config.get_instance().get("server.log.save_path", "logs")
backup_count = Config.get_instance().get("server.log.backup_count", 10)
maxMillionBytes = Config.get_instance().get("server.log.max_million_bytes", 100)
is_std = Config.get_instance().get("server.log.console", True)
console_level = Config.get_instance().get("server.log.console_level", "INFO")
filehandle_level = Config.get_instance().get("server.log.filehandle_level", "DEBUG")

logger = get_logger(
    "paraformer-webserver",
    is_std=is_std,
    stdout_level=console_level,
    stdout_format=stdout_format,
    file_level=filehandle_level,
    filehandle_format=filehandle_format,
    file_save_dir=save_path,
    max_num_of_files=backup_count,
    max_file_size=maxMillionBytes,
)
